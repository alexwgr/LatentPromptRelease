...
class PromptLatentTypeModelLatentEntropy(object):
    """
    ...
    """
    def __init__(
      self, conversation_length, num_channels, embedding_size, regularization_coefficient):
        ...
        with tf.variable_scope("classifier"):            
            ...
            query = tf.nn.xw_plus_b(tf.nn.dropout(single_batch, keep_prob=self.dropout_keep), self.W_q, self.b_q, name="query_layer")
            ...
            for channel in range(num_channels):
                norm = (1.0 / tf.cast(tf.reduce_sum(usefulness_dropout[:, :, channel, tf.newaxis],axis=1),dtype=tf.float32))
            ...
            combined_evidence = tf.reshape(combined_evidence, [self.batch_size, embedding_size * num_channels])
        ...
        with tf.name_scope("loss"):
            ...
            #entropy calculation incorrect. Need logs
            
            entropy_components = norm * tf.reduce_sum(tf.reduce_sum(usefulness_dropout * tf.math.log(usefulness_dropout), axis=2) * 
                tf.cast(self.input_masks[:, :], dtype=tf.float32), axis=1)    
            
            self.loss = tf.reduce_mean(losses) - regularization_coefficient * tf.reduce_mean(entropy_components)
